...
export default function Metas({ session }) {
  ...
  const fetchAll = async () => {
    const [movsRes, metasRes] = await Promise.all([
      supabase.from('movimientos').select('monto,tipo,fecha').eq('user_id', session.user.id),
      supabase.from('metas').select('*').eq('user_id', session.user.id).order('created_at', { ascending: false }),
    ])
    const { data: movs } = movsRes
    const { data: metasData, error } = metasRes
    if (error) { setDbError(true); setLoading(false); return }
    // Balance del mes actual
    const mesMovs = (movs || []).filter(m => {
      const f = new Date(m.fecha)
      return f.getMonth() === ahora.getMonth() && f.getFullYear() === ahora.getFullYear()
    })
    const ing = mesMovs.filter(m => m.tipo === 'ingreso').reduce((s, m) => s + Number(m.monto), 0)
    const eg  = mesMovs.filter(m => m.tipo === 'egreso').reduce((s, m) => s + Number(m.monto), 0)
    setBalance(ing - eg)
    setMetas(metasData || [])
    setLoading(false)
  }
    ...
    <div className="p-4 md:p-8 max-w-3xl mx-auto">
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p style={{ color: 'var(--text-2)', fontSize: '13px', marginTop: '4px' }}>
            Balance del mes: <strong style={{ color: balance >= 0 ? '#22c55e' : '#E63946' }}>{fmt(balance)}</strong>
          </p>
        </div>
        <button
      ...
      {metas.length === 0 ? (
        ...
        <div className="space-y-4">
          {metas.map(meta => {
            const progreso = Math.min((balance / meta.objetivo) * 100, 100)
            const onTrack = balance >= meta.objetivo * 0.5
            const completada = balance >= meta.objetivo
            return (
              <div key={meta.id} style={card}>
                <div className="flex items-start justify-between mb-3">
                  ...
                  <div className="flex items-center gap-2">
                    <span style={{ fontSize: '12px', fontWeight: '700', padding: '3px 8px', borderRadius: '6px', background: completada ? 'rgba(34,197,94,0.15)' : onTrack ? 'rgba(34,197,94,0.1)' : 'rgba(230,57,70,0.1)', color: completada ? '#16a34a' : onTrack ? '#16a34a' : '#E63946' }}>
                      {completada ? '✓ Completada' : onTrack ? '✅ En camino' : '📉 Revisar'}
                    </span>
                ...
                <div className="flex justify-between mb-2" style={{ fontSize: '13px' }}>
                  <span style={{ color: 'var(--text-2)' }}>Actual: <strong style={{ color: 'var(--text)' }}>{fmt(balance)}</strong></span>
                ...
                <div style={{ height: '10px', background: 'var(--border)', borderRadius: '5px', overflow: 'hidden' }}>
                  <div style={{ height: '100%', borderRadius: '5px', width: `${Math.max(progreso, 0)}%`, background: completada ? '#22c55e' : onTrack ? '#22c55e' : '#E63946', transition: 'width 0.7s ease' }} />
                </div>
                <p style={{ fontSize: '12px', color: 'var(--text-3)', marginTop: '6px' }}>
                  {completada ? '🎉 ¡Meta alcanzada!' : `Falta ${fmt(Math.max(meta.objetivo - balance, 0))} para la meta · ${Math.max(progreso, 0).toFixed(0)}%`}
                </p>
              </div>
            )
          })}
